        ...
        if (args.length > 0) {
            switch (args[0].toLowerCase(Locale.ROOT)) {
                ...
                case "-dfs":
                    ...
                    break;
                case "-astar":
                    String hType1 = "-distance";

                    if (args.length > 1) {
                        hType1 = args[1];
                    }

                    frontier = new FrontierBestFirst(new HeuristicAStar(initialState, hType1));
                    break;
                case "-wastar":
                    int w = 5;
                    if (args.length > 1) {
                    ...
                    break;
                case "-greedy":
                    String hType = "-distance";

                    if (args.length > 1) {
                        hType = args[1];
                    }

                    frontier = new FrontierBestFirst(new HeuristicGreedy(initialState, hType));
                    break;
                default:
                    frontier = new FrontierBFS();
